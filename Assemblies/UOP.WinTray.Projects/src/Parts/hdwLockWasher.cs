...
{
    public class hdwLockWasher : uopHardware
    {
        public override uppPartTypes BasePartType => uppPartTypes.LockWasher;

        #region Constructor

        public hdwLockWasher(uppHardwareSizes aSize = uppHardwareSizes.M10, uopHardwareMaterial aMaterial = null,int aQuantity = 1) : base(uppHardwareTypes.LockWasher, aSize, aMaterial,aQuantity) { }


        internal hdwLockWasher(uopHardware aParent, int aQuantity = 0) : base(uppHardwareTypes.LockWasher, aParent.Size, aParent.HardwareMaterial, (aQuantity > 0) ? aQuantity : aParent.Quantity)
        {
            SubPart(aParent);
        ...
        public hdwLockWasher Clone(int aQuantity = 0) => new hdwLockWasher(this, aQuantity);


        public override void UpdatePartProperties() => PartNumber = GetFriendlyName();

        public override void UpdatePartWeight() { base.Weight = Weight; }


        public bool IsEqual(hdwLockWasher aPart)
        {
            if (aPart == null) { return false; }
            if (aPart.Size != Size) { return false; }
            if (!aPart.HardwareMaterial.IsEqual(this.HardwareMaterial)) { return false; }
            return true;
        }
