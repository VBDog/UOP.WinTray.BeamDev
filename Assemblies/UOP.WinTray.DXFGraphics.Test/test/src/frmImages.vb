...
Public Class frmImages
    ...
    Private Sub Code_1()
        ...
        Try
            ...
            'lyr = _Image.Layers.AddLayer("Circles", c1, aLineWeight:=dxxLineWeights.ByDefault)
            ...
            'lyr = _Image.Layers.AddLayer("Ellipses", c1, aLineWeight:=dxxLineWeights.ByDefault)
            'c1 = dxfColors.RandomACIColor(False, c1)
            'lyr = _Image.Layers.AddLayer("Rectangles", c1, aLineWeight:=dxxLineWeights.ByDefault)



            '_Image.Draw.aLine(zRec.BottomLeft, zRec.TopRight, aColor:=dxxColors.Orange, bSuppressUCS:=True)
            Dim limline As dxeLine = _Image.Draw.aLine(New dxfVector(-3, 63.0625), New dxfVector(3, 53.59375), aColor:=dxxColors.Orange, bSuppressUCS:=True)
            Dim leg1_outside As dxeLine = _Image.Draw.aLine(New dxfVector(3, 53.59375), New dxfVector(3, 56.90625), aColor:=dxxColors.Orange, bSuppressUCS:=True)
            '_Image.Display.ZoomExtents()
            _Image.Display.ZoomExtents()
            Dim thk As Double = 0.105

            Dim ar2 As dxeArc = dxfUtils.FilletArc(limline, leg1_outside, 2 * thk)
            ...
            For i = 1 To EntCount
                ...
                If k = 1 Then
                    ...
                    _Image.Draw.aCircle(v1, s1, "Circles", c1)

                ElseIf k = 2 Then

                    _Image.Draw.aRectangle(v1, s1, s2, aRotation:=a1, aLayer:="Rectangles", aColor:=c1)

                ElseIf k >= 3 Then

                    _Image.Draw.aEllipse(v1, s1 * 2, s2 * 2, aRotation:=a1, aLayer:="Ellipses", aColor:=c1)
                    k = 0
    ...
    Private Sub Code_2()
        ...
        Dim aLayer As dxoLayer = _Image.Layers.AddLayer("TEXT", dxxColors.Blue, "Continuous", bMakeCurrent:=True)

        _Image.Layers.AddLayer("MARKERS", dxxColors.BlackWhite, "Continuous", bMakeCurrent:=False)
        ...
        Dim ttype As dxxTextTypes = dxxTextTypes.DText
        ...
        txt = "AaBbCcQqJj"
        ang = 0
        ttype = dxxTextTypes.DText
        v1.SetCoordinates(0, 0)
        tht = 100
        Dim algnm As dxxMTextAlignments = dxxMTextAlignments.Aligned
        v2 = v1.Moved(0, -1200)

        v2 = v1.Moved(10)

        dims.OrdinateH(v1, v2, -3)
        Return
        ...
        Dim aPln As dxfPlane = aTxt.Plane.Clone

        DrawCharBoxes(aTxt, bSuppressGrid:=True, bSuppressCharBoxes:=False, bSuppressBounds:=True, bSuppressChars:=False, bSuppressStrings:=True, bSuppressString:=True, bCirclesOnPathPts:=False, bCirclesOnExtentPts:=True, bDrawBaseLines:=True)

        Dim paperscale As Double = _Image.Display.ZoomExtents(bSetFeatureScale:=True)

        draw.aCircle(v1, 0.085 * paperscale, aLayer:="MARKERS", aColor:=dxxColors.Red)
        If Not IsNothing(v2) Then draw.aCircle(v2, 0.085 * paperscale, aLayer:="MARKERS", aColor:=dxxColors.Red)
    ...
    Private Sub DrawCharBoxes(aTxt As dxeText, Optional bSuppressGrid As Boolean = True, Optional bSuppressCharBoxes As Boolean = False, Optional bSuppressBounds As Boolean = True,
        ...
        If Not bSuppressGrid Then
            _Image.Layers.AddLayer("GRID", dxxColors.Orange, "Hidden2")
            _Image.Layers.AddLayer("GRIDM", dxxColors.Black, "Dot")
            v3 = v1.Clone
            For i = 1 To 3
                v2 = v3.Clone
                For j = 1 To 4
            ...
            Next
        End If
        Dim tstrings As dxoStrings = aTxt.Strings
        For Each tstring As dxoString In tstrings
            ...
            For Each schar As dxoCharacter In schars
                ...
                If bCirclesOnPathPts Then
                    draw.aCircles(schar.PathPoints, 0.01 * aTxt.TextHeight, schar.DisplaySettings.Copy(aLayer:=markDsp.LayerName, aColor:=lcolor))
                End If
            ...
            If bCirclesOnExtentPts Then
                draw.aCircles(tstring.GetBounds().Corners, 0.025 * aTxt.TextHeight, New dxfDisplaySettings(markDsp.LayerName, aColor:=dxxColors.Orange))
            End If

        Next

        Dim brec As dxfRectangle = tstrings.BoundingRectangle

        If bCirclesOnExtentPts Then
            draw.aCircles(brec.Corners, 0.03 * aTxt.TextHeight, New dxfDisplaySettings(markDsp.LayerName, aColor:=dxxColors.Yellow))
        End If
    ...
    Private Sub Code_3()
        ...
        aArc = _Image.Draw.aCircle(ipts.Item(1), 2,, dxxColors.Blue)

        ipts = aArc.Intersections(l1)
        _Image.Header.PointSize = -0.5
        ' _Image.Header.PointMode = dxxPointModes.dxfPDCircCross

        _Image.Draw.aCircles(ipts, 0.5, aColor:=dxxColors.Green)
    ...
    Private Sub Code_4()
        ...
        With _Image.DimStyle
            .ExtLineColor = dxxColors.Green
            '.ArrowHead1 = dxxArrowHeadTypes.ClosedFilled
            '.ArrowHead2 = dxxArrowHeadTypes.Integral
            .DimLineColor = dxxColors.Blue
            .TextColor = dxxColors.Magenta
            .ExtLinetype1 = "Hidden"
            .SuppressDimLine1 = False
        End With

        _Image.Display.ZoomExtents(2,, True)
        ...
        _Image.Styles.AddTextStyle("RomanD", "RomanD.shx")


        With _Image.DimStyle
            .FeatureScaleFactor = .FeatureScaleFactor * 2
            .ExtLineColor = dxxColors.Red
            '.ArrowHead1 = dxxArrowHeadTypes.BoxBlank
            '.ArrowHead2 = dxxArrowHeadTypes.ClosedBlank
            .DimLineColor = dxxColors.Magenta
            .TextColor = dxxColors.Orange
            .LinearPrecision = 1
            .ExtLinetype1 = "ByBlock"
            .SuppressDimLine1 = False
            .TextStyleName = "RomanD"
        End With
    ...
    Private Sub Code_6()
        ...
        aAr = _Image.Draw.aArc(l1.StartPt, rad,,,,, dxxColors.Blue, "Hidden")
        bAr = _Image.Draw.aArc(l1.EndPt, rad,,,,, dxxColors.Green, "Hidden")
        _Image.Display.ZoomExtents()
    ...
    Private Sub Code_7()
        ...
        aPline.Instances.Add(0, -100,,, True)
    ...
    Private Sub Code_9()
        ...
        With _Image.Draw
            ...
            _Image.Styles.AddTextStyle("MIKE", "RomanS.shx")
            _Image.Screen.PointSize = 4
            P1 = New dxfVector
            _Image.Layers.AddLayer("MIKE", dxxColors.Blue)
            ...
            '        Set aLd = .aLeader.Text(l1.MidPt, p2, "MIKE1")
            '        aLd.DimStyle.DimLineColor = dxfBlue
            '        _Image.Display.Redraw

            '        Set aDim = .aDimension_L(LinearHorizontal, l1.StartPt, l1.EndPt, 0.5, 0, "", "", False, "", 0, 0, "STANDARD")
            'GoTo Done:
            ...
            oset = 1
            '        txt = "XYXXX"
            l1 = .aLinePolarMPT(P1, ang + 90, 0.5 * lng, aLayer:="MIKE")

            l1 = .aLinePolarMPT(P1, ang, lng, aLayer:="MIKE")
            ...
            aDim = .aDim.Horizontal(l1.StartPt, l1.EndPt, -oset, toset, "", "", False, txt)
            ...
            _Image.Draw.aRectangle(aRec,,,,, dxxColors.Green)
            ...
            'Return
            aDim = .aDim.Vertical(l1.StartPt, l1.EndPt, -oset, toset, "", "", False, txt)
            aDim = .aDim.Aligned(l1.StartPt, l1.EndPt, oset, toset, "", "", False, txt)
            .aDim.TickLine(aDim, 1, -0.2, , , dxxColors.Red, "Continuous")
            .aDim.TickLine(aDim, 2, -0.2, , , , "Continuous")
            ...
            '        Set aDim = .aDimension_L(LinearVertical, l1.EndPt, l1.StartPt, -0.3, 0, "", "", False, "", 1, -0.5, "STANDARD")
            ...
            '
            '
            '        Set aDim = .aDimension_L(LinearAligned, l1.StartPt, l1.EndPt, 0.5, 0, "", "", False, "", 0, 0, "STANDARD")
            '
            ...
            '        aDim.DimStyle.SuppressExtLine1 = True
            '
            '        .Layers.Item("MIKE").Linetype = "Continuous"
...
Done:
    ...
    Private Sub Code_10()
        ...
        With _Image.Draw
            ...
            .aCircle(P1, 0.02, , dxxColors.Cyan)
            ...
            End With
            '        .Layers.AddLayer "MIKE", vbBlue + 100, , ,, True
            '        .SetTextStyleProperty "", "FontName", "Arial"

            aLine = .aLine(P1, P2)
            '        .aDimension_O OrdHorizontal, p1, p2, 0.5, -0.2, , , , , , , , , , , dxfGrey, dxfLightCyan
            '        Set aDim = aDim.OrdinateH(  p1, p2, -1.5, -0.4)
            '        Set aDim = aDim.OrdinateH(  p1, p2, -1.5, 0.4)
            '
            _Image.Display.ZoomExtents(4.5)
            '        .DimStyleOverrides.TextGap = -1 * .DimStyleOverrides.TextGap

            aDim = .aDim.OrdinateH(P1, P2, 1.5)
            .aDim.TickLine(aDim, 1, 0.25)



            aDim.DimensionPt1.Move(3, 3)
            aDim.DimStyle.ExtLineColor = dxxColors.Grey
            aDim.DimStyle.DimLineColor = dxxColors.Red
            aDim.DimStyle.TextColor = dxxColors.Orange
            aDim.DimStyle.TextGap = Math.Abs(aDim.DimStyle.TextGap)
            aDim.DimStyle.FeatureScaleFactor = 2


            aDim = .aDim.OrdinateH(P1, P2, 1.5, -0.4)

            '        Set aDim = .aDim.OrdinateV(  p1, p2, 1.5, -0.4)
            '        Set aDim = .aDim.OrdinateV(  p1, p2, 1.5, 0.4)
            aDim = .aDim.OrdinateV(P1, P2, -1.5, -0.4)
            aDim = .aDim.OrdinateV(P1, P2, -1.5, 0.4)
            aDim.DimStyle.ExtLineColor = dxxColors.Blue
            aDim.DimStyle.TextColor = dxxColors.LightYellow
            aDim.UpdateImage()
            '        aDim.Delete
            ...
            '        aDim.RotateAboutAxis aAxis, 2

            '       .aDimension_O OrdVertical, p1, p1, 3.5, -3
            '        .aDimension_O OrdVertical, p1, p1, -1.5, -0.4
...
Done:
    ...
    Private Sub Code_11()
        ...
        With _Image.Draw
            ...
            bLn = .aLinePolarSPT(p0, 10, -3, , , dxxColors.Blue, bSuppressUCS:=bSupUCS)
            gLn = .aLinePolarSPT(p0, -30, -3, , , dxxColors.Green, bSuppressUCS:=bSupUCS)
...
Done:
    ...
    Private Sub Code_13()
        ...
        With _Image.Draw
            ...
            _Image.Display.SetDisplayWindow(15, New dxfVector, 15)
            ...
            aPGon = _Image.EntityTool.CreateShape_Polygon(6, 25, New dxfVector(1, 1), 0, True, 0, , dxxColors.Blue, "Hidden", , , True)
            ...
            _Image.LinetypeLayers.Add("Center", dxxColors.Red, "Center")

            .aCircles(dPts, 0.05, , dxxColors.Yellow)
            cLns = .aCenterlines(aPGon, -0.125 * xscal, , False)
...
Done:
    ...
    Private Sub Code_14()
        ...
        With _Image.Draw
            ...
            aB = _Image.Draw.aBezier(iPts.Item(1), iPts.Item(2), iPts.Item(3), iPts.Item(4), , dxxColors.Cyan)
...
Done:
    ...
    Private Sub Code_16()
        ...
        With _Image
            ...
            .Styles.AddTextStyle("MIKE", "RomanD.shx", , 0.8)
...
Err:
    ...
    Public Sub Code_18()
        ...
        _Image.Layers.Layer("Defpoints").Visible = True
        _Image.Styles.AddTextStyle("RomanS", "RomanS.shx",, 0.6)
    ...
    Private Sub Code_19()
        ...
        shape = _Image.Draw.aPolyline(shape.Vertices, bClosed:=True, New dxfDisplaySettings(aColor:=dxxColors.Red))
        ...
        Dim aLine As dxeLine = _Image.Draw.aLine(v1, v2,, dxxColors.Blue, "Hidden")
        v2 = New dxfVector(-sz.Width / 2 / DPI, sz.Height / 2 / DPI)
        bLine = _Image.Draw.aLine(v1, v2,, dxxColors.BlackWhite, "Hidden")

        bLine.LTScale = 0.5
        bLine.UpdateImage(True)

        _Image.Draw.aEllipse(v1, 4 * rad, rad, 45,,,,, dxxColors.Green)
    ...
    Private Sub Code_20()
        ...
        _Image.Draw.aCircle(v1, 0.375, aColor:=dxxColors.Blue)
        ...
        For i As Integer = 1 To vecs.Count
            ...
            ldrtext.Add("LDRPT_" & i.ToString)
            vecs.Item(i).Tag = "LDRPT_" & i.ToString
        Next
    ...
    Private Sub Code_21()
        ...
        For i As Integer = 1 To vecs.Count
            v1 = vecs.Item(i)
            circs.Add(_Image.Draw.aCircle(v1, 0.25, aColor:=dxxColors.Cyan))
        ...
        _Image.Draw.aCircle(v1, 0.125, aColor:=dxxColors.Red)
    ...
    Private Sub Code_100()
        ...
        Dim aArcs As List(Of dxeArc) = _Image.Draw.aCircles(ipts, 2, aColor:=dxxColors.Blue)
        aArc = aArcs.Item(0)
        ipts = aArc.Intersections(l1)
    ...
    Private Sub Code_101()
        ...
        Dim ipts As colDXFVectors = l1.Intersections(aRec)
        For i As Integer = 1 To ipts.Count
            aArc = _Image.Draw.aCircle(ipts.Item(i), 2, aColor:=dxxColors.Blue)
        Next

        Dim arcs As List(Of dxeArc) = _Image.Draw.aCircles(ipts, 2, aColor:=dxxColors.Blue)
        aArc = arcs.Item(0)
        ipts = aArc.Intersections(l1)
        _Image.Header.PointSize = -0.1
        _Image.Header.PointMode = dxxPointModes.CircCross
