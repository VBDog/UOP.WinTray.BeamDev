...
#Region "Constructors"
        ...
        Friend Sub New(aEntry As TTABLEENTRY)
            MyBase.New(dxxReferenceTypes.VPORT, aEntry.Name, aGUID:=aEntry.GUID)
            If aEntry.EntryType = dxxReferenceTypes.VPORT Then Properties.CopyVals(aEntry)
        End Sub
        Public Sub New(aName As String)
            MyBase.New(dxxReferenceTypes.VPORT, aName)
            OwnerPtr = Nothing
        End Sub
        Public Sub New(aEntry As dxoViewPort)
            MyBase.New(aEntry)
        End Sub
...
#Region "Methods"
        ...
        Public Shadows Function Clone() As dxoViewPort
            Return New dxoViewPort(Me)
        End Function
